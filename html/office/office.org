...
#+BEGIN_SRC css
:root {
    --info-block-color: #268bd2;
    --info-border-color: #6c71c4;
    --success-block-color: #2aa198;
    --success-border-color: #859900;
    --warning-block-color: #b58900;
    --warning-border-color: #cb4b16;
    --error-block-color: #dc322f;
    --error-border-color: #d33682;
}
#+END_SRC
...
This is the basics for styling Agenda lists rendered by org-html.

#+BEGIN_SRC css
:root {
    --state-border-radius: 0.125em;
    --state-border-padding: 0.25em;
    --state-border: var(--border-width) var(--border-style) var(--title-color);
}
#+END_SRC

Here we style the TODO text (state) as a bordered block.

#+BEGIN_SRC css
h1 span.todo, h2 span.todo, h3 span.todo, h4 span.todo, h5 span.todo {
    border: var(--state-border);
    border-radius: var(--state-border-radius);
    padding-left: var(--state-border-padding);
    padding-right: var(--state-border-padding);
    background-color: var(--code-light-background-color);
}
#+END_SRC

And we move tags to the right and de-emphasize.

#+BEGIN_SRC css
h1 span.tag, h2 span.tag, h3 span.tag, h4 span.tag, h5 span.tag {
    float:right;
...
div.NOTE, div.INFO, div.WARNING, div.ERROR, div.SUCCESS, div.FAILURE {
    ...
    border: var(--border-width-thick) var(--border-style) var(--info-border-color);
